...
export function buildBarcodeReadPrompt(){return `BARCODE PHOTO READER
Read only the UPC, EAN or ISBN printed beneath the barcode in the attached image.
Return JSON only using this exact shape: {"barcode":""}.
The value must contain only the barcode characters: digits, or a final X for ISBN-10.
Do not identify the product, guess missing digits, repair a partly hidden code, or return any other text.
If every character is not clearly readable, return {"barcode":""}.`;}
